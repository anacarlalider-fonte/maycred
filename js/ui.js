  ...
  function renderDashboardGestor(container) {
    ...
    container.appendChild(
      el(
        ...
        'Cada card mostra o quanto da meta de rentabilidade (comissão no caixa) já foi acumulado: só entram propostas com tabela e % definidos (valor financiado × % da tabela). Projeta análise + pago conforme o status. O valor do objetivo em R$ não aparece aqui — só o progresso até 100%. Detalhes em Produção.'
      )
    );
    ...
    const stCfg = global.MaycredData.getState();
    function cfgFld(lb, inp) {
      const f = el('div', 'ui-field');
      f.appendChild(el('span', 'ui-field__label', lb));
      f.appendChild(inp);
      return f;
    }
    const extra = el('div', 'ui-dash-gestor-extra');
    extra.appendChild(el('h2', 'ui-section__title', 'Rentabilidade e acesso'));
    extra.appendChild(
      el(
        'p',
        'ui-muted',
        'Retenção sobre produção bruta e custo operacional do mês alimentam KPIs e export CSV.',
      ),
    );
    const fRent = el('form', 'ui-form-grid ui-form-grid--2');
    const inpSpreadPct = el('input', 'ui-input');
    inpSpreadPct.type = 'number';
    inpSpreadPct.step = '0.01';
    inpSpreadPct.min = '0';
    inpSpreadPct.max = '100';
    inpSpreadPct.value = String((stCfg.config.spreadBanco || 0) * 100);
    const inpCustoOp = el('input', 'ui-input');
    inpCustoOp.type = 'number';
    inpCustoOp.step = '0.01';
    inpCustoOp.min = '0';
    inpCustoOp.value = String(
      stCfg.config.custoOperacionalMes != null ? stCfg.config.custoOperacionalMes : 0,
    );
    fRent.appendChild(cfgFld('Retenção banco/parceiro (% sobre produção bruta)', inpSpreadPct));
    fRent.appendChild(cfgFld('Custos operacionais do mês (R$)', inpCustoOp));
    const sbRent = el('button', 'ui-btn ui-btn--primary', 'Salvar rentabilidade');
    sbRent.type = 'submit';
    sbRent.style.gridColumn = '1 / -1';
    fRent.appendChild(sbRent);
    fRent.addEventListener('submit', function (e) {
      e.preventDefault();
      const pct = parseFloat(inpSpreadPct.value);
      const custo = parseFloat(inpCustoOp.value);
      global.MaycredData.setConfig({
        spreadBanco: Math.min(1, Math.max(0, (Number.isNaN(pct) ? 0 : pct) / 100)),
        custoOperacionalMes: Number.isNaN(custo) ? 0 : Math.max(0, custo),
      });
      toast('Rentabilidade salva.', 'success');
    });
    extra.appendChild(fRent);

    const f4 = el('form', 'ui-form-grid');
    const ps = el('input', 'ui-input');
    ps.type = 'password';
    ps.placeholder = 'Nova senha';
    const pc = el('input', 'ui-input');
    pc.type = 'password';
    pc.placeholder = 'Confirmar senha';
    f4.appendChild(cfgFld('Nova senha do gestor', ps));
    f4.appendChild(cfgFld('Confirmação', pc));
    const sb4 = el('button', 'ui-btn ui-btn--primary', 'Alterar senha');
    sb4.type = 'submit';
    f4.appendChild(sb4);
    f4.addEventListener('submit', function (e) {
      e.preventDefault();
      ...
      global.MaycredData.setConfig({ senhaGestor: ps.value });
      ps.value = '';
      pc.value = '';
      toast('Senha alterada.', 'success');
    });
    extra.appendChild(el('h3', 'ui-config-block__title ui-dash-gestor-extra__sub', 'Senha do gestor'));
    extra.appendChild(f4);
  ...
  function renderProducao(container) {
    ...
    page.appendChild(
      el(
        'p',
        'ui-muted',
        'Mês no topo. Produção bruta = soma do valor financiado das propostas. Rentabilidade (meta) = valor financiado × % da tabela vinculada; só entra o que tem tabela cadastrada na proposta. Colunas Análise/Pago/Produção são em R$ de comissão. Objetivo mensal em Configurações → Metas do mês (meta rentabilidade). Lançamentos legados seguem regra antiga por linha.',
      ),
    );
    ...
    function tdMoneyReadonly(label, val) {
    ...
    function paint() {
      ...
      const table = el('table', 'ui-table ui-table--responsive ui-table--producao-resumo');
      ...
      order.forEach(function (R) {
        ...
        avt.appendChild(el('div', 'ui-vend-cell__disc', v.disc || '—'));
        av.appendChild(avt);
        ...
        tr.appendChild(tdMoneyReadonly('Produção bruta (R$)', row.producaoBruta));
        tr.appendChild(tdMoneyReadonly('Rentab. análise (R$)', row.analise));
        tr.appendChild(tdMoneyReadonly('Rentab. paga (R$)', row.pago));
        tr.appendChild(tdMoneyReadonly('Rentab. total (R$)', row.total));
